...
macro(CGET_PARSE_OPTIONS name)
    set(options NO_FIND_PACKAGE REGISTRY NOSUBMODULES PROXY ALLOW_SYSTEM NUGET_USE_STATIC NO_FIND_VERSION PROGRAM LIBRARY)
...
function(CGET_DIRECT_GET_PACKAGE name)
    ...
    if (NOT EXISTS ${STAGING_DIR})
        ...
        if ("" STREQUAL "${CHECKOUT_TAG}" AND "" STREQUAL "${ARGS_COMMIT_ID}" AND ARGS_GIT)
            CGET_EXECUTE_PROCESS(COMMAND ${GIT_EXECUTABLE} log --oneline -n1 WORKING_DIRECTORY ${STAGING_DIR} OUTPUT_VARIABLE THIS_COMMIT)

            CGET_EXECUTE_PROCESS(COMMAND ${GIT_EXECUTABLE} fetch WORKING_DIRECTORY ${STAGING_DIR} OUTPUT_QUIET ERROR_QUIET)
...
function(CGET_HAS_DEPENDENCY name)
    ...
    if(ARGS_ALLOW_SYSTEM)
        CGET_FIND_DEPENDENCY(${ARGS_FINDNAME} ${ARGS_CMAKE_VERSION} QUIET ${ARGS_FIND_OPTIONS})
    ...
    if (NOT ARGS_NO_FIND_PACKAGE)
        ...
        CGET_FIND_DEPENDENCY(${ARGS_FINDNAME} ${ARGS_CMAKE_VERSION} REQUIRED ${ARGS_FIND_OPTIONS})
