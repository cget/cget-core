...
if (NOT DEFINED CGET_CORE_DIR)
  ...
  CGET_ADD_CUSTOM_TARGET(cget-rebuild-packages)  
endif()
...
if(MSVC OR MINGW)
    ...
    if("${CMAKE_GENERATOR}" MATCHES "Win64" OR "${CMAKE_GENERATOR}" MATCHES "MinGW Makefiles")
        set(CGET_ARCH "x64")
...
macro(CGET_NUGET_BUILD name version)    
    ...
    #check semi-std path naming
    SET(PATH_HINT "${OUTPUTDIR}/*.windesktop.msvcstl.dyn.rt-dyn.*/lib/native/${CGET_MSVC_RUNTIME}/windesktop/msvcstl/dyn/rt-dyn/")
    SET(CHECK_STD_PATHS "${PATH_HINT}/${CGET_ARCH_WIN_NAME}/Debug" 
                        "${PATH_HINT}/${CGET_ARCH_WIN_NAME}/Release" 
                        "${PATH_HINT}/${CGET_ARCH}/Debug" 
                        "${PATH_HINT}/${CGET_ARCH}/Release")
    CGET_GLOB_MULTIDIR(DLLS "${CHECK_STD_PATHS}" "dll" )    
    if(NOT DLLS)
        file(GLOB_RECURSE DLLS "${OUTPUTDIR}" "${OUTPUTDIR}/*.dll")
    endif()
    
    CGET_GLOB_MULTIDIR(LIBS "${CHECK_STD_PATHS}" "lib" )            
    if(NOT LIBS)    
...
macro(CGET_PARSE_OPTIONS name)
  ...
  set(oneValueArgs GITHUB GIT HG SVN URL NUGET_PACKAGE NUGET_VERSION VERSION FINDNAME COMMIT_ID REGISTRY_VERSION OPTIONS_FILE CMAKE_VERSION)
...
function(CGET_HAS_DEPENDENCY name)
  ...
  CGET_FILE_CONTENTS("${BUILD_DIR}/.built" BUILD_CACHE_VAL)
    CGET_MESSAGE(5 "Build status ${BUILD_CACHE_VAL} vs ${Build_Hash}")
