...
CGET_MESSAGE(13 "Core dir: ${CGET_CORE_DIR} / ${CMAKE_SOURCE_DIR}")
CGET_MESSAGE(13 "IS_ROOT_DIR: ${CGET_IS_ROOT_DIR}")
...
macro(CGET_REGISTER_INSTALL_DIR INSTALL_DIR)
  if (EXISTS ${INSTALL_DIR})
    if (NOT CGET_IS_SCRIPT_MODE)
	  ...
	  list(APPEND CMAKE_INCLUDE_PATH  ${INSTALL_DIR} ${INSTALL_DIR}/include)
    ...
    CGET_MESSAGE(15 "Could not register install dir ${INSTALL_DIR}")
  endif()
endmacro()
...
function(CGET_WRITE_CGET_SETTINGS_FILE)
    set(CGET_PASSTHRU_VARS
        ...
        CMAKE_SYSTEM_VERSION CMAKE_POLICY_VERSION_MINIMUM
        ...
        CMAKE_ANDROID_ARCH_ABI CMAKE_ANDROID_STL_TYPE CMAKE_ANDROID_NDK ANDROID_ALLOW_UNDEFINED_SYMBOLS CMAKE_ANDROID_ARM_MODE)
...
SET(REL_BUILD_DIR "build-${CGET_CONFIGURATION_SUFFIX}")
if (CMAKE_BUILD_TYPE)
...
macro(CGET_PARSE_OPTIONS name)
    set(options NO_FIND_PACKAGE REGISTRY NOSUBMODULES PROXY ALLOW_SYSTEM NUGET_USE_STATIC NO_FIND_VERSION
            PROGRAM LIBRARY SIMPLE_BUILD INCLUDE_ONLY)
    ...
    IF (ARGS_SIMPLE_BUILD_SOURCE_FILES OR ARGS_SIMPLE_BUILD_HEADER_FILES OR ARGS_INCLUDE_ONLY)
    ...
    SET(BUILD_HASH_STRING "${name} ${ARGS_OPTIONS} ${ARGS_NOSUBMODULES} ${CGET_CORE_VERSION} ${ARGS_NUGET_PACKAGE} ${ARGS_NUGET_USE_STATIC} ${ARGS_NUGET_VERSION} ${ARGS_BREW_PACKAGE}  ${ARGS_VERSION} ${ARGS_COMMIT_ID} ${ARGS_REGISTRY_VERSION}${SUBMODULE_VERSION}${CMAKE_FIND_ROOT_PATH}")
    ...
    if (CGET_RETRIEVE_MECHANISM MATCHES NUGET)
        # Keep nuget packages in a separate directory so they don't interfere with cross-platform builds
        string(APPEND REPO_DIR_SUFFIX ${ARGS_NUGET_PACKAGE})
    endif()
...
macro(CGET_SIMPLE_BUILD name)
    ...
    file(COPY ${REPO_DIR}/include DESTINATION ${CGET_INSTALL_DIR})
    IF(NOT ARGS_INCLUDE_ONLY)
...
function(CGET_CLEAR_CMAKE_CACHE dir)
    CGET_MESSAGE(3 "Cleaning cmake cache from ${dir}")
    FILE(REMOVE "${dir}/CMakeCache.txt")
endfunction()
...
macro(CGET_BUILD_CMAKE name)
    ...
    STRING(REPLACE "\\" "/" CMAKE_MAKE_PROGRAM_CLEAN "${CMAKE_MAKE_PROGRAM}")
    set(CMAKE_OPTIONS ${ARGS_OPTIONS}
            -DCMAKE_INSTALL_RPATH_USE_LINK_PATH:BOOL=ON
            -C${CGET_BIN_DIR}/load.cmake
            -C${CMAKE_BINARY_DIR}/cget-env.cmake
            -DCMAKE_MAKE_PROGRAM=${CMAKE_MAKE_PROGRAM_CLEAN}
            ${USER_INCLUDE_FILE}
            -G${CMAKE_GENERATOR}
    ...
    if (NOT "${CMAKE_TOOLCHAIN_FILE}" STREQUAL "")
        ...
        STRING(REPLACE "\\" "/" sub_toolchain_file "${sub_toolchain_file}")
        list(APPEND CMAKE_OPTIONS -DCMAKE_TOOLCHAIN_FILE=${sub_toolchain_file})
    endif ()

    IF("${CMAKE_GENERATOR}" STREQUAL "Unix Makefiles")
        SET(ADDITIONAL_GENERATOR_OPTIONS "--" "-j4")
    ENDIF()

    if (DEFINED CMAKE_BUILD_TYPE AND NOT "${CMAKE_BUILD_TYPE}" STREQUAL "")
        CGET_CLEAR_CMAKE_CACHE("${RELEASE_BUILD_DIR}")
        CGET_CLEAR_CMAKE_CACHE("${BUILD_DIR}")
        ...
        CGET_EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} --build . --target install --config ${CMAKE_BUILD_TYPE} ${ADDITIONAL_GENERATOR_OPTIONS} WORKING_DIRECTORY "${BUILD_DIR}")
        ...
        if (RELEASE_BUILD_DIR)
            ...
            CGET_EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} --build . --target install --config Release ${ADDITIONAL_GENERATOR_OPTIONS} WORKING_DIRECTORY "${RELEASE_BUILD_DIR}")
        endif ()
    else ()
        CGET_CLEAR_CMAKE_CACHE("${BUILD_DIR}")
        # Set up the packages
        ...
        foreach (configuration ${CGET_BUILD_CONFIGS})
            ...
            CGET_EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} --build . --target install --config ${configuration} ${ADDITIONAL_GENERATOR_OPTIONS} WORKING_DIRECTORY ${BUILD_DIR})
        endforeach ()
    endif ()
endmacro()
...
function(CGET_FORCE_BUILD name)
    ...
    foreach (config_variant configure config bootstrap makefile Makefile)
	    ...
	    if(NOT "${config_variant}" STREQUAL "Makefile")
            ...
            CGET_EXECUTE_PROCESS(COMMAND make ${ARGS_OPTIONS} WORKING_DIRECTORY ${TEMP_SRC_DIR})
...
macro(CGET_HAS_DEPENDENCY name)
    ...
    if(NOT "${${ARGS_FINDNAME}_FOUND}")
      ...
      LIST(REMOVE_AT CGET_CURRENT_CHAIN -1)      
    endif()
